...
public class INIHelper extends ArrayList {
    private File userFileName;

    /**
     * Constructor IniFile
     *
     * Creates a IniFile object for an application. Create assigns the FileName parameter
    ...
    public String ReadString(String Section, String key, String defaultValue) {
        ...
        if (ValuePosition(Section, key) > 0) {
            ...
            value = get(ValuePosition(Section, key)).toString().substring(strLen);
        }
        return value;
    }
    ...
    public ArrayList ReadSectionValues(String Section) {
        ...
        if (this.SectionPosition(Section) > -1) {
            for (int i=start; i < size(); i++) {
                s = get(i).toString().substring(get(i).toString().indexOf("=")+1);
                if (s.startsWith("[") && s.endsWith("]")) {
                    break;
                } else {
                    myList.add(s);
    ...
    public int ReadInteger(String Section, String key, int defaultValue) {
        ...
        if (ValuePosition(Section, key) > 0) {
            int strLen = key.length()+1;
            value = Integer.parseInt(get(ValuePosition(Section, key)).toString().substring(strLen));
        }
        return value;
    }
    ...
    public Float ReadFloat(String Section, String key, Float defaultValue) {
        ...
        if (ValuePosition(Section, key) > 0) {
            int strLen = key.length()+1;
            value = Float.valueOf(get(ValuePosition(Section, key)).toString().substring(strLen));
        }
        return value;
    }
    ...
    public boolean ReadBool(String Section, String key, boolean defaultValue) {
        ...
        if (ValuePosition(Section, key) > 0) {
            int strLen = key.length()+1;
            value = Boolean.valueOf(get(ValuePosition(Section, key)).toString().substring(strLen));
        }
        return value;
    }
    ...
    private void addToList(String Section, String key, String value) {
        if (this.SectionExist((Section))) {
            if (this.ValueExist(Section, key)) {
                ...
                add(getLastValuePosition(Section), value);
            }
        } else {
            add("["+Section+"]");
            add(value);
    ...
    private int getLastValuePosition(String Section) {
        int pos = 0;
        int start = this.SectionPosition(Section)+1;
        if (this.SectionPosition(Section) > -1) {
            for (int i=start; i < size(); i++) {
                if (get(i).toString().startsWith("[") && get(i).toString().endsWith("]")) {
                    break;
                } else {
                    pos++;
                }
    ...
    public void RenameSection(String oldSection, String newSection) {
        for (int i=0; i < size(); i++) {
            if (get(i).toString().equals("["+oldSection+"]")) {
                set(i, "["+newSection+"]");
                //UpdateFile();
                break;
            }
